...
Responder.prototype.respond = function respond(method) {
   ...
   .then(null, this.onUnhandledError)
   .done(this.sendResponse);
...
BaseResponder.prototype.setHeader = function setHeader(name, value, overwrite) {
...
BaseResponder.prototype.setCookie =
...
BaseResponder.prototype.setResponseCode =
...
BaseResponder.prototype.sendResponse = function sendResponse(body) {
   // set response code
   ...
   for (var i = 0; i < keys.length; ++i) {
      var name = keys[i];
      var cookie = this.cookiesToSet_[name];
      var str = cookie.serialize(name, cookie.value, cookie.opts);
